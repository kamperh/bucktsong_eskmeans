...
"""
Get the Buckeye English and NCHLT Xitsonga data.
...
def get_tokens_from_fa(fa_fn):
    ...
    with open(fa_fn, "r") as f:
        for line in f:
            ...
            end = int(round(float(end) * 100)) + 1  # exclusive
            utt = utt.replace("_", "-")
            if "nchlt" in utt:
                corpus, lang, speaker, utt = utt.split("-")
                utt = "{}_{}-{}-{}".format(speaker, corpus, lang, utt)
            else:
                utt = utt[:3] + "_" + utt[3:]
...
def get_word_gt_dicts(subset, utt_ids):
    ...
    for key in tqdm(sorted(utt_ids)):
        speaker, utt, interval = key.split("_")
        utt = speaker + "_" + utt
...
def get_phone_gt_dicts(subset, utt_ids):
    ...
    if subset == "devpart1" or subset == "zs":
        fa_fn = path.join(forced_alignment_dir, "buckeye_english.phn")
    elif subset == "xitsonga":
...
def main():
    ...
    key = list(word_gt_bounds_dict)[0]
