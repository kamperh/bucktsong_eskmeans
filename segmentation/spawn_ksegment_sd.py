...
from get_data_sd import devpart1_speakers, zs_speakers, xitsonga_speakers
from ksegment import default_options_dict
from spawn_segment_sd_eval import segment_sd_eval

# devpart1_speakers = ["s02", "s04", "s05"]  # temp
sd_options = ["rnd_seed", "min_duration", "init_am_n_iter", "segment_n_iter"]
...
def main():
    ...
    print(
        str(sum([1 for i in exit_codes if i == 0])) + " out of " +
        str(len(procs)) + " succeeded"
        )
