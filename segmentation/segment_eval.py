...
def init_segmenter_from_output(pickle_fn, data_dir=None):
    ...
    if "fb_type" in options_dict and options_dict["fb_type"] in ["unigram", "bigram"]:
        # Bigram segmenter
        segmenter = bigram_segment.segment(options_dict, suppress_pickling=True)
    elif "n_iter_inbetween_kmeans" in options_dict:
        # K-means segmenter
        segmenter = ksegment.ksegment(options_dict, suppress_pickling=True)
    else:
        # Unigram segmenter
...
def analyze_segmentation(unsup_landmarks, unsup_transcript,
    ...
    analysis["unsup_transcript_true"] = unsup_transcript_seq
...
def segment_eval(pickle_fn, level, suppress_plot=True):
    ...
    d = path.split(pickle_fn)[0]
    data_dir = path.join(
        "data", path.normpath(path.split(d)[0]).replace("kmodels" + os.sep,
        "").replace("models" + os.sep, "")
        )
    ...
    print("Avg. no. tokens in cluster:", np.mean(sizes))
    print("Std. no. tokens in cluster:", np.var(sizes))
    ...
    total_time = 0
    if not am_init_record is None:
        print()
        if "log_marg" in am_init_record and len(am_init_record["log_marg"]) > 0:
            print("Initial acoustic model sampling final log P(X, z) =", am_init_record["log_marg"][-1])
        elif "sum_neg_sqrd_norm" in am_init_record and len(am_init_record["sum_neg_sqrd_norm"]) > 0:
            print(
                "Initial acoustic model sampling final sum_neg_sqrd_norm = " +
                str(am_init_record["sum_neg_sqrd_norm"][-1])
                )
        total_time += sum(am_init_record["sample_time"])
        result_dict["avg_am_init_sample_time"] = np.mean(am_init_record["sample_time"])
    if not segmenter_record is None:
        print()
        if "log_prob_z" in segmenter_record and len(segmenter_record["log_prob_z"]) > 0:
        ...
        elif "sum_neg_sqrd_norm" in segmenter_record and len(segmenter_record["sum_neg_sqrd_norm"]) > 0:
            print("Final sum_neg_sqrd_norm =", segmenter_record["sum_neg_sqrd_norm"][-1])
            print("Final sum_neg_len_sqrd_norm =", segmenter_record["sum_neg_len_sqrd_norm"][-1])
            result_dict["sum_neg_len_sqrd_norm"] = segmenter_record["sum_neg_len_sqrd_norm"][-1]            
        total_time += sum(segmenter_record["sample_time"])
        result_dict["avg_segmenter_sample_time"] = np.mean(segmenter_record["sample_time"])
    ...
    if not segmenter_record is None and not suppress_plot and "log_marg" in segmenter_record:
